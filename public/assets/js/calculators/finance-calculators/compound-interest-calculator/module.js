...
const principalField = document.querySelector('#ci-principal-field');
const rateField = document.querySelector('#ci-rate-field');
const timeField = document.querySelector('#ci-time-field');
const contributionField = document.querySelector('#ci-contribution-field');
...
const previewPanel = document.querySelector('#ci-preview');
const staleNote = document.querySelector('#ci-stale-note');
...
const growthChartEndingBalance = document.querySelector('[data-ci="chart-ending-balance"]');
const growthChartPrincipalBase = document.querySelector('[data-ci="chart-principal-base"]');
const growthChartInterestTotal = document.querySelector('[data-ci="chart-interest-total"]');
...
const chartScaleGroup = document.querySelector('[data-button-group="ci-chart-scale"]');
...
const metadata = {
  ...
  canonical: 'https://calchowmuch.com/finance-calculators/compound-interest-calculator/',
  pageSchema,
...
function formatWholeFieldValue(value) {
  return fmt(value, {
    minimumFractionDigits: 0,
...
function formatDecimalFieldValue(value) {
...
function ensureGrowthChartHydration() {
  if (growthChartHydrated || !growthChartSection || !growthChartCanvas) {
    return;
  }
  ...
  loadGrowthChartModule()
    ...
    .catch((error) => {
      ...
      setGrowthStatus('Unable to load chart. You can still use the projection table above.');
    });
}
...
function renderGrowthChart(data) {
  if (!growthChartSection || !growthChartCanvas) {
    return;
  }
  ...
  if (!growthChartHydrated || !growthChartRenderer?.update) {
    setGrowthStatus('Chart initializes after first paint and when this section is near view.');
    return;
  }
...
function calculate() {
  ...
  try {
    ...
    summaryDiv.innerHTML =
      `<article class="fi-metric-card">` +
      `<span class="fi-metric-label">Total interest</span>` +
      `<strong class="fi-metric-value">${totalInterestStr}</strong>` +
      `</article>` +
      `<article class="fi-metric-card">` +
      `<span class="fi-metric-label">Total contributions</span>` +
      `<strong class="fi-metric-value">${totalContribStr}</strong>` +
      `</article>` +
      `<article class="fi-metric-card">` +
      `<span class="fi-metric-label">Compounding</span>` +
      `<strong class="fi-metric-value">${compoundingStr}</strong>` +
      `</article>`;

    /* Snapshot rows */
    if (snapPrincipal) {
      snapPrincipal.textContent = principalStr;
    }
    if (snapRate) {
      snapRate.textContent = rateStr;
    }
    if (snapTime) {
      snapTime.textContent = timeStr;
    }
    if (snapCompounding) {
    ...
    if (snapPeriodicRate) {
      snapPeriodicRate.textContent = `${periodicRateStr}%`;
    }
    if (snapTotalPeriods) {
      snapTotalPeriods.textContent = totalPeriodsStr;
    }
