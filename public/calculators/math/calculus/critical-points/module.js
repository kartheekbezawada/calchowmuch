...
class CriticalPointsFinder {
  ...
  tokenizeExpression(expr) {
    ...
    while (i < cleaned.length) {
      ...
      // Identifier (variables and functions: x, sin, cos, etc.)
      if (/[a-zA-Z]/.test(ch)) {
        let id = ch;
        i++;
  ...
  toRPN(tokens) {
    ...
    const isFunction = id => ['sin', 'cos', 'tan', 'sqrt', 'ln', 'log', 'exp', 'abs'].includes(id);
  ...
  // Evaluate an RPN expression at a given x value
  evaluateRPN(rpn, xValue) {
    const stack = [];
    ...
    for (const token of rpn) {
      if (token.type === 'number') {
        ...
        const fn = functionsMap[token.value];
        if (!fn) {
...
function evaluateExpr(expr, x) {
  // Create a temporary instance to use the safe evaluator
  const tempFinder = new CriticalPointsFinder(expr);
  return tempFinder.evaluateFunction(expr, x);
}
