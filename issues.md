...
|----------|----------|--------|----------|-------------|------------|------------|------------|---------------|
| ISS-001 | UI/Layout | Resolved | P1 | Bouncing UI elements when navigating between calculators | Scrollbar appearing/disappearing based on content height. No explicit scrollbar styling was defined, and `overflow-y: auto` allowed scrollbar to toggle visibility. | Added explicit scrollbar CSS styling. Changed all scrollable panes to `overflow-y: scroll` with `scrollbar-gutter: stable` to always reserve space. Added visible scrollbar with subtle gray color. | 2026-01-15 | 2026-01-15 |
