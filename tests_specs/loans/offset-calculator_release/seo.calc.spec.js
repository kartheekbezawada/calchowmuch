...
test.describe('Offset Calculator SEO', () => {
  test('OFFSET-SEO-1: metadata, schema, FAQ parity, sitemap, and section order', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Offset Calculator');

    await expect(page.locator('#off-section-lifetime')).toHaveCount(1);
    await expect(page.locator('#off-section-amortization')).toHaveCount(1);
    await expect(page.locator('#off-section-how-to-guide')).toHaveCount(1);
    await expect(page.locator('#off-section-faq')).toHaveCount(1);
    await expect(page.locator('#off-section-notes')).toHaveCount(1);
    await expect(page.locator('#off-section-notes')).toContainText('Last updated:');

    const sectionOrder = await page.evaluate(() => {
      const ids = [
        'off-section-lifetime',
        'off-section-amortization',
        'off-section-how-to-guide',
        'off-section-faq',
        'off-section-notes',
      ];
    ...
    const visibleQuestions = await page
      .locator('#loan-offset-explanation #off-section-faq .bor-faq-card h4')
      .allTextContents();
    ...
    expect(await sitemapResponse.text()).toContain('/loan-calculators/offset-mortgage-calculator/');
  });
});
