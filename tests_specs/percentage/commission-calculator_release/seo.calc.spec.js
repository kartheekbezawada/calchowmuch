...
test.describe('Commission Calculator SEO', () => {
  test('COMM-TEST-SEO-1: metadata, schema, sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Commission Calculator');

    const canonical = page.locator('link[rel="canonical"]');
    await expect(canonical).toHaveCount(1);
    const canonicalHref = await canonical.getAttribute('href');
    expect(canonicalHref).toBe('https://calchowmuch.com/percentage-calculators/commission-calculator/');
    ...
    expect(sitemapText).toContain('/percentage-calculators/commission-calculator/');
  });
});
