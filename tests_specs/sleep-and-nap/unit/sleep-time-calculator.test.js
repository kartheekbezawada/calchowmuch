...
describe('Sleep Time Calculator - SLEEP-TEST-U-1: Calculation Logic', () => {
  it('should calculate bedtimes for wake-up mode (6:00 AM)', () => {
    const wakeTime = new Date(2026, 0, 2, 6, 0);
    const recommendations = calculateSleepRecommendations({ mode: 'wake', date: wakeTime });

    expect(recommendations.map((rec) => rec.cycles)).toEqual(SLEEP_CYCLES);
    recommendations.forEach((rec) => {
      const diffMinutes = (wakeTime.getTime() - rec.sleepTime.getTime()) / 60000;
  ...
  it('should calculate wake times for fall-asleep mode (11:30 PM)', () => {
    ...
    const recommendations = calculateSleepRecommendations({ mode: 'sleep', date: sleepTime });
  ...
  it('should return empty recommendations for invalid dates', () => {
    ...
    expect(calculateSleepRecommendations({ mode: 'wake', date: invalid })).toEqual([]);
  });
});

describe('Sleep Time Calculator - SLEEP-TEST-U-2: Defaults and Rounding', () => {
