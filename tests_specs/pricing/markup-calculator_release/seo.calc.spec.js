...
test.describe('Markup Calculator SEO', () => {
  test('MARKUP-TEST-SEO-1: metadata, structured data, sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Markup Calculator');

    const canonical = page.locator('link[rel="canonical"]');
    await expect(canonical).toHaveCount(1);
    const canonicalHref = await canonical.getAttribute('href');
    expect(canonicalHref).toBe('https://calchowmuch.com/pricing-calculators/markup-calculator/');
    ...
    expect(sitemapText).toContain('/pricing-calculators/markup-calculator/');
  });
});
