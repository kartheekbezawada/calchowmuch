...
test.describe('Compound Interest Calculator', () => {
  test('CI-TEST-E2E-1: user journey and projection table frequency toggle', async ({ page }) => {
    ...
    const baselineText = await page.locator('#ci-result').textContent();

    await page.fill('#ci-rate', '6');
    await page.fill('#ci-time', '2');
    await page.fill('#ci-contribution', '100');
    await expect(page.locator('#ci-stale-note')).toBeVisible();
    await expect(page.locator('#ci-result')).toHaveText(String(baselineText || ''));

    await page.click('#ci-calc');
    await expect(page.locator('#ci-stale-note')).toBeHidden();
    ...
    const annualResultValue = parseNumber(await page.locator('#ci-result').textContent());
    ...
    await expect(page.locator('#ci-stale-note')).toBeVisible();
    expect(parseNumber(await page.locator('#ci-result').textContent())).toBeCloseTo(
      annualResultValue,
      2
    );
    ...
    await page.click('#ci-calc');
    await expect(page.locator('#ci-stale-note')).toBeHidden();
    expect(parseNumber(await page.locator('#ci-result').textContent())).toBeGreaterThan(
      annualResultValue
    );
    ...
    await expect(
      page.locator('[data-button-group="ci-chart-scale"] button[data-value="linear"]')
    ).toHaveClass(/is-active/);
    await page.click('[data-button-group="ci-chart-scale"] button[data-value="log"]');
    await expect(
      page.locator('[data-button-group="ci-chart-scale"] button[data-value="log"]')
    ).toHaveClass(/is-active/);
  });
});
