...
test.describe('math/derivative seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Derivative Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.deriv-exp-section');
    ...
    await expect(root.locator('.deriv-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/calculus/derivative/');
  });
});
