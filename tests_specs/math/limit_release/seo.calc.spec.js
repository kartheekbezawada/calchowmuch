...
test.describe('math/limit seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Limit Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.limq-exp-section');
    ...
    await expect(root.locator('.limq-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/calculus/limit/');
  });
});
