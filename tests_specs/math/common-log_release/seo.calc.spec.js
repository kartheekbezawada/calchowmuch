...
test.describe('math/common-log seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Common Log Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.clog-exp-section');
    ...
    await expect(root.locator('.clog-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/log/common-log/');
  });
});
