...
test.describe('math/hypothesis-testing seo', () => {
  test('metadata, explanation contract, schema parity, and sitemap are present', async ({
    ...
    const explanation = page.locator('#hypothesis-explanation');
    await expect(explanation.locator('h2')).toHaveText('Hypothesis Testing Practical Guide');
    ...
    expect(faqNode.mainEntity[0].name).toBe('What does reject H0 actually mean?');

    const sitemapResponse = await page.request.get('/sitemap.xml');
    expect(sitemapResponse.ok()).toBeTruthy();
    expect(await sitemapResponse.text()).toContain('/math/statistics/hypothesis-testing/');
  });
});
