...
test.describe('math/log-scale seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(canonical).toHaveAttribute('href', 'https://calchowmuch.com/math/log/log-scale/');

    const h1 = page.locator('h1');
    await expect(h1).toHaveCount(1);
    await expect(h1).toHaveText('Log Scale Converter');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.lscale-exp-section');
    ...
    await expect(root.locator('.lscale-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/log/log-scale/');
  });
});
