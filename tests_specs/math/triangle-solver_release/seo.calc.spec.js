...
test.describe('math/triangle-solver seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Triangle Solver');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.tri-exp-section');
    ...
    await expect(root.locator('.tri-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/trigonometry/triangle-solver/');
  });
});
