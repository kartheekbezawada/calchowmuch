...
test.describe('math/factoring seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    await page.goto('/math/algebra/factoring/');

    await expect(page).toHaveTitle('Factoring Calculator | Polynomial Factoring Steps | CalcHowMuch');

    const canonical = page.locator('link[rel="canonical"]');
    await expect(canonical).toHaveCount(1);
    await expect(canonical).toHaveAttribute('href', 'https://calchowmuch.com/math/algebra/factoring/');

    const h1 = page.locator('h1');
    await expect(h1).toHaveCount(1);
    await expect(h1).toHaveText('Factoring Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.factor-exp-section');
    ...
    const methodNotesIndex = h3Texts.indexOf('Method Notes');
    ...
    expect(methodNotesIndex).toBeGreaterThan(-1);
    ...
    await expect(root.locator('.factor-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/algebra/factoring/');
  });
});
