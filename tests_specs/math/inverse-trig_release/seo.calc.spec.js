...
test.describe('math/inverse-trig seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Inverse Trig Functions Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.inv-exp-section');
    ...
    await expect(root.locator('.inv-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/trigonometry/inverse-trig/');
  });
});
