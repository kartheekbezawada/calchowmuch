...
test.describe('math/anova seo', () => {
  test('metadata, explanation contract, schema parity, and sitemap are present', async ({
    ...
    const explanation = page.locator('#anova-explanation');
    await expect(explanation.locator('h2')).toHaveText('ANOVA Calculator Practical Guide');
    ...
    expect(faqNode.mainEntity[0].name).toBe('Can I use ANOVA with only two groups?');

    const sitemapResponse = await page.request.get('/sitemap.xml');
    expect(sitemapResponse.ok()).toBeTruthy();
    expect(await sitemapResponse.text()).toContain('/math/statistics/anova/');
  });
});
