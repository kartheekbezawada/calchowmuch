...
test.describe('math/statistics seo', () => {
  test('metadata, explanation contract, schema parity, and sitemap are present', async ({
    ...
    const explanation = page.locator('#statistics-explanation');
    await expect(explanation.locator('h2')).toHaveText('Statistics Calculator Practical Guide');
    ...
    expect(faqNode.mainEntity[0].name).toBe(
      'When should I use this route instead of mean-median-mode-range?'
    );

    const sitemapResponse = await page.request.get('/sitemap.xml');
    expect(sitemapResponse.ok()).toBeTruthy();
    expect(await sitemapResponse.text()).toContain('/math/statistics/');
  });
});
