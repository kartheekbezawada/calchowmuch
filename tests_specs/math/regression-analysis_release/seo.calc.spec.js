...
test.describe('math/regression-analysis seo', () => {
  test('metadata, explanation contract, schema parity, and sitemap are present', async ({
    ...
    const explanation = page.locator('#regression-explanation');
    await expect(explanation.locator('h2')).toHaveText('Regression Analysis Practical Guide');
    ...
    expect(faqNode.mainEntity[0].name).toBe('What does R-squared tell me?');

    const sitemapResponse = await page.request.get('/sitemap.xml');
    expect(sitemapResponse.ok()).toBeTruthy();
    expect(await sitemapResponse.text()).toContain('/math/statistics/regression-analysis/');
  });
});
