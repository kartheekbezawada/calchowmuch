...
test.describe('math/exponential-equations seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Exponential Equation Solver');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.expq-exp-section');
    ...
    await expect(root.locator('.expq-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/log/exponential-equations/');
  });
});
