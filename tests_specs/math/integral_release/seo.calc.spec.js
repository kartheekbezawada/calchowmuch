...
test.describe('math/integral seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Integral Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.intg-exp-section');
    ...
    await expect(root.locator('.intg-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/calculus/integral/');
  });
});
