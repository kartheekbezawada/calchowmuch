...
test.describe('math/trig-functions seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Trigonometric Functions Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.trig-exp-section');
    ...
    await expect(root.locator('.trig-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/trigonometry/trig-functions/');
  });
});
