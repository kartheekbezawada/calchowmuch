...
test.describe('math/unit-circle seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(
      'Unit Circle Calculator | Trig Values, Reference Angles & Quadrants'
    );
    ...
    await expect(h1).toHaveText('Unit Circle Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.unit-exp-section');
    ...
    const exactValueIndex = h3Texts.indexOf('Exact Value Notes');
    ...
    expect(exactValueIndex).toBeGreaterThan(-1);
    ...
    await expect(root.locator('.unit-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/trigonometry/unit-circle/');
  });
});
