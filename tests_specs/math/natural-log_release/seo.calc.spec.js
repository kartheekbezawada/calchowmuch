...
test.describe('math/natural-log seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Natural Log Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.ln-exp-section');
    ...
    await expect(root.locator('.ln-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/log/natural-log/');
  });
});
