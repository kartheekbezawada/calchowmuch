...
test.describe('math/law-of-sines-cosines seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(page).toHaveTitle(
      'Law of Sines and Cosines Calculator | Triangle Solver | CalcHowMuch'
    );
    ...
    await expect(h1).toHaveText('Law of Sines and Cosines Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.law-exp-section');
    ...
    const formulaIndex = h3Texts.indexOf('Formula Guide');
    ...
    await expect(root.locator('.law-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/trigonometry/law-of-sines-cosines/');
  });
});
