...
test.describe('math/polynomial-operations seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(page).toHaveTitle('Polynomial Operations Calculator | Algebra Tool');
    ...
    const explanationSections = root.locator('.poly-exp-section');
    ...
    const operationNotesIndex = h3Texts.indexOf('Operation Notes');
    ...
    expect(operationNotesIndex).toBeGreaterThan(-1);
    ...
    await expect(root.locator('.poly-related-card')).toHaveCount(3);
