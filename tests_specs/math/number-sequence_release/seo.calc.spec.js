...
test.describe('math/number-sequence seo', () => {
  test('metadata, explanation contract, schema parity, and sitemap are present', async ({ page }) => {
    ...
    const explanation = page.locator('#sequence-explanation');
    await expect(explanation.locator('h2')).toHaveText('Number Sequence Calculator Practical Guide');
    await expect(explanation.locator('.faq-card')).toHaveCount(6);
    await expect(explanation.locator('.sequence-related-card')).toHaveCount(3);
    ...
    expect(faqNode.mainEntity[0].name).toBe('How many terms do I need?');

    const sitemapResponse = await page.request.get('/sitemap.xml');
    expect(sitemapResponse.ok()).toBeTruthy();
    expect(await sitemapResponse.text()).toContain('/math/number-sequence/');
  });
});
