...
test.describe('math/log-properties seo', () => {
  test('metadata, explanation contract, FAQ depth, schema parity, and sitemap', async ({ page }) => {
    ...
    await expect(h1).toHaveText('Logarithm Properties Calculator');

    const root = page.locator('.calculator-page-single');
    const explanationSections = root.locator('.lprop-exp-section');
    ...
    await expect(root.locator('.lprop-related-card')).toHaveCount(3);
    ...
    expect(await sitemapResponse.text()).toContain('/math/log/log-properties/');
  });
});
