...
| HDR-3.3 | Header height is fixed (per UI-3.1); adding new elements must preserve the premium-dark spacing tokens from `UNIVERSAL_REQUIREMENTS.md`. | P1 |
...
- Keep the header height consistent: use `unit-spacing-4` (from `UNIVERSAL_REQUIREMENTS.md`) for padding and ensure the header does not push the page height beyond `100vh`.
...
- Use `aria-label`s on icon-only links (if any) and ensure contrast meets WCAG 2.1 AA via the premium-dark palette (`UNIVERSAL_REQUIREMENTS.md`).
