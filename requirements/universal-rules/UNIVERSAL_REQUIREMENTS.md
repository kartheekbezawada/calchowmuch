...
#### UUI-FDP-001 — Form Density Principle (P0)
Calculation Pane content must be structured so that a user can:
- Reach a first meaningful result quickly
- Avoid unnecessary vertical scrolling before calculation
- Understand required vs optional inputs at a glance

**Interpretation:**
- Vertical scrolling is allowed, but must be a last resort, not the default.
- If a user must scroll before they can reasonably calculate, the form density is considered suboptimal.

#### UUI-FDP-002 — Core vs Optional Inputs (P0)
All calculator inputs must be classified as either:
- Core inputs (required to produce the primary result), or
- Optional / Advanced inputs (enhancements, refinements, or what-ifs)

**Requirements:**
- Core inputs must be visible without scrolling whenever feasible.
- Optional inputs must not block or delay calculation.
- Optional inputs should be grouped under an Advanced / Optional section when input count exceeds comfortable density.

#### UUI-FDP-003 — Progressive Disclosure (P0)
When the number of inputs exceeds a comfortable density, calculators must use progressive disclosure rather than vertical stacking.

**Allowed mechanisms:**
- Collapsible “Advanced options” section
- Logical grouping (e.g., payments, taxes, extras)
- Contextual reveal (e.g., switching Amount/Percent changes label, not layout)

**Disallowed:**
- Hiding required inputs behind collapsible sections
- Auto-expanding optional sections without user intent
- Multi-step wizards (unless explicitly required by a calculator spec)

#### UUI-FDP-004 — Row Efficiency (P1)
Where inputs are conceptually related and do not reduce clarity, they may share a row inside the Calculation Pane.
...
**Constraints:**
- Shared rows must not introduce ambiguity
- Labels must remain clear and directly associated with inputs
- Switching input modes must not change row structure

#### UUI-FDP-005 — Scroll Bar Usage (P1)
Scroll bars inside the Calculation Pane are allowed, but must be intentional and justified.

**Guidance:**
- Scrolling caused by optional inputs is acceptable.
- Scrolling required to access core inputs should trigger a review.
- If scrolling delays time-to-first-result, the form must be restructured.

This rule exists because excessive scrolling:
- Increases bounce risk
- Increases time-to-first-interaction
- Reduces perceived performance and confidence

#### UUI-FDP-006 — No Compression at the Cost of Clarity (P0)
Improving form density must never:
- Reduce label clarity
- Shrink hit targets
- Remove spacing needed for readability
- Overload a single row with unrelated fields

This is not a mandate to “pack everything tighter.”

#### UUI-FDP-007 — Change Control
Any calculator that violates UUI-FDP rules:
- Must be amended when touched next (opportunistic improvement)
- Must not trigger global UI redesigns unless explicitly scoped

**Agents must not:**
- Redesign unrelated calculators
- Introduce new layout systems
- Apply pixel/breakpoint changes without an explicit requirement

**Compliance Checklist (for Agents)**
Before shipping or modifying a calculator, agents must self-check:

 - Are all core inputs discoverable immediately?
 - Is scrolling required before calculation?
 - Are optional inputs clearly separated?
 - Are shared rows logical and stable?
 - Is clarity preserved over compactness?

If any answer is “no”, the calculator must be restructured.

**Why This Belongs in Universal Requirements**
- It is behavioral, not stylistic
- It protects engagement without freezing UI evolution
- It gives agents guardrails without micromanaging layout
- It prevents silent UX regressions across calculators

**Recommended Placement:**
Add this as a new section in: requirements/universal/UNIVERSAL_REQUIREMENTS.md, Section: UI & Interaction Principles
