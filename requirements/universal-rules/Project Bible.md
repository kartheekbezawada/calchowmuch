...
- **Explanation Quality Contract:** Output insight sections may appear first; required SERP explanation block order is Intent-led heading, then `How to Guide`, then FAQ, then `Important Notes` as the final section.
...
- **Important notes:** Include required keys: `Last updated` (Month Year), `Accuracy`, calculator-relevant disclaimer, calculator-specific `Assumptions`, and exact `Privacy` statement.
- **SERP guide consistency:** Keep reusable guide/notes presentation consistent across calculators (bullet-list notes, no container box, typography baseline by default).
- **Typography baseline:** Guide `H3` 16px, guide `H4` 14px, body/list 14px unless route-specific REQ overrides.
- **Theme fidelity:** Guide/notes use calculator font inheritance; `Important Notes` key labels use `rgba(186, 230, 253, 0.98)`.
...
- [ ] Intent-led structure present in required SERP block: `Intent heading -> How to Guide -> FAQ -> Important Notes` with `Important Notes` last.
- [ ] Important Notes includes all required key lines and updated `Last updated` month/year.
