# Release Sign-Off — REL-20260227-014

## 1) Release Identity

| Field | Value |
| :--- | :--- |
| **Release ID** | REL-20260227-014 |
...
| **Pane Layout Evidence Path** | public/config/navigation.json (`id: polynomial-operations` -> `paneLayout: single`) |
...
| **thinContentArtifactPath** | `test-results/content-quality/scoped/math/polynomial-operations.json` |
| **Owner** | Codex |
| **Date** | 2026-02-27 |
...
| **A4** | **JS Discipline** | Operation modes + deterministic divide-by-zero behavior validated | Pass |
...
- `npm run lint` -> Pass
- `CLUSTER=math CALC=polynomial-operations npm run test:calc:unit` -> Pass
- `CLUSTER=math CALC=polynomial-operations npm run test:calc:e2e` -> Pass
- `CLUSTER=math CALC=polynomial-operations npm run test:calc:seo` -> Pass
- `CLUSTER=math CALC=polynomial-operations npm run test:calc:cwv` -> Pass
- `CLUSTER=math CALC=polynomial-operations npm run test:schema:dedupe -- --scope=calc` -> Pass
- `npm run test:isolation:scope` -> Pass
- `npm run test:cluster:contracts` -> Pass

### Scoped CWV Strict Profiles (Calculator Release)
Artifact path: `test-results/performance/scoped-cwv/math/polynomial-operations.json`

- `mobile_strict`: CLS `0.0000`, LCP `1184ms`, blocking CSS duration `0ms`, blocking CSS requests `0`
- `desktop_strict`: CLS `0.0159`, LCP `1300ms`, blocking CSS duration `0ms`, blocking CSS requests `0`

### Exceptions
| ID | Issue | Severity | Owner |
| :--- | :--- | :--- | :--- |
| EX-REL-20260227-014-01 | `test:isolation:scope` reported non-blocking route lookup skip for aggregate `algebra` source prefix | Low | Codex |
