# Release Sign-Off — REL-20260227-001

## 1) Release Identity

| Field | Value |
| :--- | :--- |
| **Release ID** | REL-20260227-001 |
...
| **thinContentArtifactPath** | not emitted (pilotExcluded=1 in scoped SEO run) |
| **Owner** | Codex |
| **Date** | 2026-02-27 |
...
| **A2** | **CSS Arch** | No `@import` in route source; optimized route asset delivery | Pass |
...
| **A5** | **Caching** | Versioned calculator CSS/JS URLs (`v=20260228`) | Pass |
...
| **C1** | **Field Metrics** | LCP/CLS/INP policy met via scoped strict CWV profiles | Pass |
...
| **F** | **Animation** | Transform/opacity-based route animations | Pass |
| **G1** | **First Load** | Old-design flash mitigated via route asset-manifest delivery + direct route stylesheet link | Pass |
...
| :--- | :--- | :--- | :--- |
| **X1** | **Investigate** | LCP element and strict profile metrics captured | Pass |
...
| Metric | Value | Status |
| :--- | :--- | :--- |
| **LCP** | 1568 ms (`mobile_strict`) | Pass |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `mobile_strict` (CPU 3x + Slow 4G, cache disabled) | 0 | 1568 | 0 | 0 | Pass |
| `desktop_strict` (CPU 6x + Slow 4G, cache disabled) | 0.0005 | 1788 | 0 | 0 | Pass |
...
| `thinContentGrade` | N/A |
| `thinContentHardFlags` | none |
| `thinContentArtifactPath` | not emitted (pilotExcluded=1) |

### Exceptions
| ID | Issue | Severity | Owner |
| :--- | :--- | :--- | :--- |
| EX-REL-20260227-001-01 | `test:isolation:scope` requires `ALLOW_SHARED_CONTRACT_CHANGE=1` due approved shared-contract change (`public/config/asset-manifest.json`) | Low | Codex |
