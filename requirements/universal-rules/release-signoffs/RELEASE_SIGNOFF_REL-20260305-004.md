...
### Executed Gates
- Target generation:
  - `TARGET_CALC_ID=credit-card-minimum-payment node scripts/generate-mpa-pages.js`
- Scoped SEO gate:
  - `CLUSTER=credit-cards CALC=credit-card-minimum-payment npm run test:calc:seo` -> Pass
- Scoped schema dedupe gate:
  - `CLUSTER=credit-cards CALC=credit-card-minimum-payment npm run test:schema:dedupe -- --scope=calc` -> Pass (`scanned=1 changed=0 parseErrors=0 unresolved=0`)

### Skipped Gates (Mode-Locked)
- `CLUSTER=credit-cards CALC=credit-card-minimum-payment npm run test:calc:unit` (skipped by human request)
- `CLUSTER=credit-cards CALC=credit-card-minimum-payment npm run test:calc:e2e` (skipped by human request)
- `CLUSTER=credit-cards CALC=credit-card-minimum-payment npm run test:calc:cwv` (skipped by human request)
...
### Evidence Paths
- Route artifact: `public/credit-card-calculators/credit-card-minimum-payment-calculator/index.html`
- Thin-content scoped artifact emitted by SEO runner: `test-results/content-quality/scoped/credit-cards/credit-card-minimum-payment.json` (pilotExcluded summary mode)
...
SEO + schema scoped release checks are complete for `credit-cards/credit-card-minimum-payment` under human-locked maintenance mode.
