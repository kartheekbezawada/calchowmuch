...
| CWV | `CLUSTER=loans CALC=how-much-can-i-borrow npm run test:calc:cwv` | Fail | `test-results/performance/scoped-cwv/loans/how-much-can-i-borrow.json` (`blockingCssDurationMs` exceeds strict budget) |
...
| Isolation Scope | `CLUSTER=loans ROUTE=/loan-calculators/how-much-can-i-borrow/ npm run test:isolation:scope` | Fail | Script flags in-scope calculator source paths as `Another calculator source changed` due route-map source-prefix mismatch |
...
| Pane layout proof (for `calc_exp`) | Route remains single-pane `calc_exp`; e2e layout checks pass with deterministic worker setting |

Notes:
- `Your Income Capacity` is now a 4-slice donut visual and placed side-by-side with `Rate Scenarios` inside a single shared explanation container.
- Existing test-critical hooks are preserved (`data-bor="cap-*"`, `data-bor="legend-*"`, scenario table IDs/selectors).
- To keep scope constraints, route-bundle rebuild artifacts outside approved paths were reverted immediately.
...
| RISK-002 | Strict scoped CWV budget fails due blocking CSS duration (`mobile_strict` and `desktop_strict` > 800ms). | High | Requires shared-contract/perf-budget strategy (outside approved file scope) to reduce strict budget duration. |
| RISK-003 | `test:isolation:scope` strict check fails for this route because source-prefix mapping flags this calculator path as `another calculator` in current script logic. | Medium | Requires shared-contract/script fix or release-policy exception for this route. |
